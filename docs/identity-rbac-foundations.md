...
- Scope owner-operators to their own company, assigned drivers, units, loads, documents, incidents, and settlements.
- Scope drivers to assigned trips and driver workflows only.
- Support both owner-operator drivers and CargoFi/company drivers.
- Preserve historical driver assignment records when drivers move between owner-operators or company fleet operations.
- Prevent driver accounts without an active operational assignment from accessing dispatch workflows.
...
create unique index idx_driver_assignments_one_current
...
create index idx_driver_assignments_driver_history on driver_assignments(driver_id, starts_at desc);
create index idx_driver_assignments_oo_current on driver_assignments(owner_operator_id, driver_id) where ends_at is null;
create index idx_driver_assignments_company_current on driver_assignments(tenant_id, driver_id) where assignment_type = 'company' and ends_at is null;
create index idx_driver_assignments_unit_current on driver_assignments(unit_id) where ends_at is null;
...
- Driver operational ownership is resolved through `driver_assignments`, not by relying only on `drivers.owner_operator_id`.
- A current driver assignment is the row where `driver_assignments.ends_at is null`.
- Owner-operator drivers have `assignment_type = 'owner_operator'` and a required `owner_operator_id`.
- CargoFi/company drivers have `assignment_type = 'company'` and no `owner_operator_id`.
- `unit_id` on `driver_assignments` may link a driver to a current truck/unit assignment, including company units where `units.owner_operator_id` is null.
- Drivers with no current assignment may remain in the database for compliance/history, but they must not receive operational access.
...
- Historical assignment rows should be used for audits, compliance review, and explaining who a driver belonged to at a point in time; they should not rewrite historical load ownership.

## Driver Assignment Rules

- Creating a driver user account is not enough to grant mobile workflow access.
- A driver must have an active `user_profiles` row, a `driver_users` mapping, and a current `driver_assignments` row.
- Admins can create and close driver assignments.
- Owner-operators can only see drivers whose current assignment points to their `owner_operator_id`.
- Company drivers are visible to admins and company-fleet workflows only, not to owner-operators.
- When a driver moves from one owner-operator to another, or from owner-operator fleet to company fleet, close the old assignment by setting `ends_at`, then create a new assignment.
- The partial unique index prevents more than one current assignment per driver.
- If a current assignment is closed and no replacement exists, driver mobile access should be blocked immediately except for a minimal account/support screen.

## Migration Order

1. Add enum and identity mapping tables without changing existing login behavior.
2. Add `driver_assignments` with historical date ranges and one-current-assignment enforcement.
3. Backfill an initial admin profile for the existing CargoFi operator account.
4. Backfill current driver assignments from existing `drivers.owner_operator_id` where present.
5. For drivers without `owner_operator_id`, require an admin decision before granting operational access: assign to an owner-operator or create a company assignment.
6. Add owner-operator and driver user mapping rows only when real accounts are created.
7. Add server helpers that read the Supabase session and resolve the current profile plus current driver assignment.
8. Keep the existing `dispatch_auth` cookie accepted for admin-only legacy access during migration.
9. Move API routes from cookie-only checks to centralized authorization helpers route by route.
10. Add RLS policies only after API authorization is in place and verified.
11. Remove the legacy shared password cookie once all admin access uses Supabase Auth.
...
- Drivers: Supabase user plus `user_profiles.role = 'driver'`, `driver_users` mapping, and a current `driver_assignments` row.

Do not encode authorization only in JWT custom claims. Claims can help performance later, but database-backed profile and assignment lookups should be authoritative during rollout.
...
- `src/lib/auth/assignments.ts`: resolve current driver assignment and historical assignment lookups.
...
- Owner-operator: can access resources where the resource belongs to their `owner_operator_id`, including drivers whose current assignment points to that owner-operator.
- Company fleet/admin workflows: can access company drivers where current `driver_assignments.assignment_type = 'company'`.
- Driver: can access assigned trips through `loads.driver_id` or `load_drivers.driver_id` only if they also have a current assignment; can upload/view documents and incidents only for assigned trips or their own driver profile.
- Unassigned driver: no operational API access, even if the Supabase account and `driver_users` row exist.
...
- Admin plus owner-operator scoped: owner-operator ledger, settlement previews, owner-operator documents, loads assigned to owner-operator, drivers currently assigned to owner-operator.
- Admin plus company-fleet scoped: company drivers, company units, company-driver assignments, company-load workflows.
...
- On each mobile API request, confirm the driver still has a current assignment before returning operational data.
- If a driver assignment is closed mid-session, show a minimal blocked/support state rather than assigned trip data.
...
- Add `driver_assignments` with current and historical assignment support.
- Add server auth helper interfaces.
- Preserve current shared-password admin access.
- Add tests for helper decisions using mocked profiles/resources/assignments.
...
- Add admin-only assignment management endpoints before exposing driver mobile access.
...
- Add scoped API reads for owner-operator loads, documents, ledger, settlements, and currently assigned drivers.
- Keep UI minimal and operational; do not redesign admin screens.

### Phase 4: Driver Mobile Workflows

- Add driver account mapping.
- Require current driver assignment for all operational driver APIs.
...
### Phase 5: Company Fleet Workflows

- Add admin/company workflows for drivers assigned directly to CargoFi.
- Support company unit assignment through `driver_assignments.unit_id`.
- Keep company-driver resources separate from owner-operator scoped views.

### Phase 6: RLS Tightening and Legacy Removal
...
- Add a migration for `app_role`, `driver_assignment_type`, `user_profiles`, `owner_operator_users`, `driver_users`, `driver_assignments`, and indexes.
- Add placeholder auth helper files with typed interfaces and no behavior change to existing routes.
- Include a backfill plan for existing drivers with `owner_operator_id` and a blocked state for drivers with no assignment.
...
- Adding `driver_assignments` is slightly more schema than storing only `owner_operator_id` on `drivers`, but it supports company drivers, historical movement, and cleaner authorization.
- Enforcing no unassigned operational driver access may temporarily block some existing drivers until admins classify them as owner-operator or company drivers.
...
- `supabase/`: identity tables, driver assignment tables, indexes, RLS policies, audit tables, incident tables.
- `src/lib/supabase.ts`: Supabase server/client setup changes for real sessions.
- `src/lib/auth/`: new centralized auth, assignment, and authorization helpers.
...
- `src/app/api/loads/**`, `src/app/api/owner-operators/**`, `src/app/api/drivers/**`: ownership, assignment, and trip access checks.
