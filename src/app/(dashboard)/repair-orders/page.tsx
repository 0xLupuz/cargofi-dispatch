...
const STATUS_COLORS: Record<string,string> = {
  ...
  delivered_incomplete:'bg-[rgba(58,182,144,0.1)] text-[#3ab690]',
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="flex items-center justify-between px-6 py-4 border-b border-[#21262d] flex-shrink-0">
        <div className="flex items-center gap-2">
          <Wrench className="w-5 h-5 text-[#3ab690]" />
          <h1 className="text-white font-semibold text-lg">Repair Orders</h1>
          <span className="text-xs text-[#484f58] bg-[#161b22] rounded-full px-2 py-0.5">{ros.length}</span>
        </div>
      ...
      {/* Filters */}
      <div className="flex items-center gap-3 px-6 py-3 border-b border-[#21262d] flex-shrink-0">
        <div className="flex items-center gap-2 text-xs text-gray-500">
          <Calendar className="w-4 h-4" />
          <input type="date" value={from} onChange={e => setFrom(e.target.value)} className="bg-[#161b22] border border-[#30363d] rounded px-2 py-1 text-xs text-white focus:outline-none focus:border-[#3ab690]" />
          <span>to</span>
          <input type="date" value={to} onChange={e => setTo(e.target.value)} className="bg-[#161b22] border border-[#30363d] rounded px-2 py-1 text-xs text-white focus:outline-none focus:border-[#3ab690]" />
        </div>
        <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)} className="bg-[#161b22] border border-[#30363d] rounded-lg px-3 py-1.5 text-xs text-white focus:outline-none focus:border-[#3ab690] cursor-pointer">
          <option value="">All Statuses</option>
          {Object.entries(STATUS_LABELS).map(([k,v]) => <option key={k} value={k}>{v}</option>)}
        </select>
        <div className="relative">
          <Search className="w-3.5 h-3.5 absolute left-2.5 top-1/2 -translate-y-1/2 text-gray-500" />
          <input type="text" placeholder="RO #..." value={search} onChange={e => setSearch(e.target.value)} className="bg-[#161b22] border border-[#30363d] rounded-lg pl-8 pr-3 py-1.5 text-xs text-white placeholder-gray-500 focus:outline-none focus:border-[#3ab690] w-40" />
        </div>
      </div>
      ...
      <div className="flex-1 overflow-auto">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-gray-800/60">
            {ros.map(ro => {
              ...
              return (
                <tr key={ro.id} className="hover:bg-[#161b22]/30 transition-colors group">
                  <td className="px-4 py-3 font-mono font-semibold text-[#3ab690] text-sm">{ro.ro_number}</td>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      ...
                      <button onClick={() => del(ro.id, ro.ro_number)} disabled={deleting===ro.id} className="text-[#484f58] hover:text-red-400"><Trash2 className="w-3.5 h-3.5" /></button>
                    </div>
                  </td>
                </tr>
              )
        ...
        {ros.length === 0 && <div className="flex flex-col items-center justify-center h-40 text-[#484f58]"><Wrench className="w-8 h-8 mb-2 opacity-30" /><p className="text-sm">No repair orders in this date range</p></div>}
      </div>
