...
function Breakdown({ load }: { load: any }) {
  ...
  return (
    <div className="mt-3 flex flex-wrap gap-2 text-xs">
      ...
      <Pill label={`Dispatch (${load.dispatch_fee_pct ?? 13}%)`} value={`-$${fmt(dspFee)}`} color="text-orange-400" />
...
function Pill({ label, value, color, highlight }: { label: string; value: string; color: string; highlight?: boolean }) {
  return (
    <div className={`rounded-lg px-3 py-1.5 ${highlight ? 'bg-emerald-500/10 border border-emerald-500/20' : 'bg-gray-800/60'}`}>
                  ...
                  <div className="flex items-start justify-between">
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs text-gray-600">{fmtDate(load.pickup_date)}</span>
                      </div>
                      <p className="text-gray-300 text-sm mt-0.5">{load.broker_name ?? '—'}</p>
                    </div>
                    <div className="flex items-center gap-2">
                      <Link href={`/settlements/${load.id}`} className="flex items-center gap-1.5 text-xs text-gray-400 hover:text-white border border-gray-700 hover:border-gray-500 rounded-lg px-3 py-1.5 transition-colors">
...
function HistoryTab() {
  ...
  return (
    <div className="flex flex-col h-full">
      {/* Search + summary */}
      <div className="px-6 py-3 border-b border-gray-800 flex items-center gap-4 flex-shrink-0">
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-800/60">
              {loads.map(load => {
                ...
                return (
                  <tr key={load.id} className="hover:bg-gray-800/40 cursor-pointer transition-colors group">
                    <td className="px-4 py-3 font-mono text-orange-400 font-semibold">{load.load_number}</td>
                    ...
                    <td className="px-4 py-3 text-orange-400">-${fmt(dspFee)}</td>
        ...
        <div className="flex items-center gap-1 bg-gray-800/60 rounded-lg p-1">
          <button
            ...
            className={`flex items-center gap-1.5 rounded-md px-3 py-1.5 text-xs font-semibold transition-colors ${
              tab === 'pending' ? 'bg-orange-500/20 text-orange-400' : 'text-gray-500 hover:text-gray-300'
            }`}
          >
          ...
          <button
            ...
            className={`flex items-center gap-1.5 rounded-md px-3 py-1.5 text-xs font-semibold transition-colors ${
              tab === 'history' ? 'bg-orange-500/20 text-orange-400' : 'text-gray-500 hover:text-gray-300'
            }`}
          >
