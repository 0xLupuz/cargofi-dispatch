...
const STATUS_STYLE: Record<string, { bg: string; text: string; label: string }> = {
  ...
  rate_con:    { bg: 'bg-blue-500/15',        text: 'text-blue-400',   label: 'Rate Con'   },
  ...
  delivered:   { bg: 'bg-orange-500/15',      text: 'text-orange-400', label: 'Delivered'  },
  ...
  return (
    <div className="h-full overflow-auto px-6 py-6 space-y-6 max-w-5xl">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { label: 'Active Loads',      value: data?.activeLoads ?? 0,             sub: 'confirmed + in transit',        icon: Truck,      color: 'text-orange-400', border: 'border-orange-500/20', bg: 'bg-orange-500/5', fmt: false },
          ...
          { label: 'Avg Rate / Load',   value: data?.avgRate ?? 0,                 sub: `${data?.totalLoads ?? 0} total loads`,        icon: TrendingUp,  color: 'text-blue-400',   border: 'border-blue-500/20',   bg: 'bg-blue-500/5',    fmt: true  },
        ].map(c => (
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden">
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-800">
          ...
          <button onClick={() => router.push('/loads')} className="text-xs text-orange-400 hover:text-orange-300 transition-colors flex items-center gap-1">
        ...
        {data?.recentLoads?.length > 0 ? (
              ...
              {data.recentLoads.map((l: any) => {
                ...
                return (
                  <tr key={l.id} onClick={() => router.push('/loads')}
                    ...
                    <td className="px-4 py-3 font-mono text-orange-400 font-semibold text-xs">{l.load_number}</td>
          ...
          <div className="px-5 py-10 text-center text-gray-600 text-sm">
            No loads yet — <button onClick={() => router.push('/loads')} className="text-orange-400 hover:underline">create your first load</button>
          </div>
        )}
      </div>
