    ...
    <div className="h-full flex flex-col">
      ...
      <div className="flex items-center justify-between px-6 py-4 border-b border-[#21262d] flex-shrink-0">
        <div className="flex items-center gap-3">
          <Store className="w-5 h-5 text-[#3ab690]" />
          ...
          <div className="flex items-center gap-1 ml-2 bg-[#161b22] rounded-lg p-1">
            <button
              onClick={() => setShowActive(true)}
              className={`text-xs px-3 py-1 rounded-md transition-colors ${showActive ? 'bg-[#3ab690] text-white' : 'text-gray-400 hover:text-white'}`}
            >Active</button>
            <button
        ...
        <div className="flex items-center gap-3">
          <div className="relative">
            ...
            <input
              ...
              className="bg-[#161b22] border border-[#30363d] rounded-lg pl-9 pr-3 py-2 text-sm text-white placeholder-gray-500 focus:outline-none focus:border-[#3ab690] w-52"
            />
          </div>
          <button
      ...
      <div className="flex-1 overflow-auto">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-gray-800/60">
            {filtered.map(v => (
              <tr key={v.id} className="hover:bg-[#161b22]/30 transition-colors group">
                ...
                <td className="px-4 py-3 text-gray-400 text-xs">
                  {v.address ?? '—'}
                  {v.address2 && <span className="block text-[#484f58]">{v.address2}</span>}
                </td>
                ...
                <td className="px-4 py-3 text-gray-400 text-xs">
                  {v.telephone ?? '—'}
                  {v.telephone_ext && <span className="text-[#484f58]"> x{v.telephone_ext}</span>}
                </td>
                ...
                <td className="px-4 py-3">
                  <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                    ...
                    <button onClick={() => del(v.id, v.name)} disabled={deleting === v.id} className="text-[#484f58] hover:text-red-400">
