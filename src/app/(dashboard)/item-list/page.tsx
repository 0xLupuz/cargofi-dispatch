    ...
    <div className="h-full flex flex-col">
      ...
      <div className="flex items-center justify-between px-6 py-4 border-b border-gray-800 flex-shrink-0">
        ...
        <div className="flex items-center gap-3">
          <select value={typeFilter} onChange={e => setTypeFilter(e.target.value)} className="bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-orange-500 cursor-pointer">
            <option>All</option><option value="P">Parts</option><option value="L">Labor</option>
          </select>
          <div className="relative">
            <Search className="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-gray-500" />
            <input type="text" placeholder="Code, item, description..." value={search} onChange={e => setSearch(e.target.value)} className="bg-gray-800 border border-gray-700 rounded-lg pl-9 pr-3 py-2 text-sm text-white placeholder-gray-500 focus:outline-none focus:border-orange-500 w-56" />
          </div>
      ...
      {/* Alphabetical filter */}
      <div className="px-6 py-2 border-b border-gray-800 flex items-center gap-1 flex-wrap flex-shrink-0">
        <button onClick={() => setAlphaFilter('')} className={`text-xs px-2 py-1 rounded transition-colors ${!alphaFilter ? 'bg-orange-500 text-white' : 'text-gray-500 hover:text-white hover:bg-gray-800'}`}>All</button>
        {ALPHA.map(a => (
          <button key={a} onClick={() => setAlphaFilter(alphaFilter === a ? '' : a)} className={`text-xs px-2 py-1 rounded transition-colors ${alphaFilter === a ? 'bg-orange-500 text-white' : 'text-gray-600 hover:text-white hover:bg-gray-800'}`}>{a}</button>
        ))}
      </div>
      ...
      <div className="flex-1 overflow-auto">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-gray-800/60">
            {filtered.map(it => (
              <tr key={it.id} className="hover:bg-gray-800/30 transition-colors group">
                <td className="px-4 py-2.5 font-mono font-semibold text-orange-400 text-xs">{it.code}</td>
                <td className="px-4 py-2.5">
                  <span className={`text-xs rounded-full px-2 py-0.5 font-semibold ${it.item_type === 'L' ? 'bg-amber-500/10 text-amber-400' : 'bg-blue-500/10 text-blue-400'}`}>{it.item_type === 'L' ? 'Labor' : 'Part'}</span>
                </td>
                ...
                <td className="px-4 py-2.5">{it.taxable ? <span className="text-xs text-emerald-400">Yes</span> : <span className="text-xs text-gray-600">No</span>}</td>
                <td className="px-4 py-2.5"><div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity"><button onClick={() => setModal({ open:true, item:it })} className="text-gray-400 hover:text-white"><Pencil className="w-3.5 h-3.5" /></button><button onClick={() => del(it.id)} disabled={deleting===it.id} className="text-gray-600 hover:text-red-400"><Trash2 className="w-3.5 h-3.5" /></button></div></td>
              </tr>
            ))}
          </tbody>
        </table>
        {filtered.length === 0 && <div className="flex flex-col items-center justify-center h-40 text-gray-600"><List className="w-8 h-8 mb-2 opacity-30" /><p className="text-sm">{search ? 'No items found' : 'No items in catalog yet'}</p></div>}
      </div>
