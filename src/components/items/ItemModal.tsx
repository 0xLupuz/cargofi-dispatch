...
export default function ItemModal({ item, onClose, onSaved }: Props) {
  ...
  const [form, setForm] = useState({
    code: item?.code ?? '', item_type: item?.item_type ?? 'P',
    item: item?.item ?? '', sub_item: item?.sub_item ?? '',
    description: item?.description ?? '', charge_rate: item?.charge_rate ?? '0',
    taxable: item?.taxable ?? false, active: item?.active ?? true,
  })
  ...
  async function save(e: React.FormEvent) {
    ...
    payload.taxable = form.taxable; payload.active = form.active
    ...
    <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
      <div className="bg-gray-900 border border-gray-700 rounded-xl w-full max-w-md max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-800">
          <h2 className="text-white font-semibold">{isEdit ? 'Edit Item' : 'New Item'}</h2>
          <button onClick={onClose}><X className="w-5 h-5 text-gray-400 hover:text-white" /></button>
        </div>
        <form onSubmit={save} className="px-6 py-5 space-y-4">
          <div className="grid grid-cols-3 gap-3">
            <div><label className={lbl}>Code *</label><input className={inp} value={form.code} onChange={e => set('code', e.target.value.toUpperCase())} required /></div>
            ...
            <div><label className={lbl}>{form.item_type === 'L' ? '$/Hour' : 'Unit Price'}</label><input className={inp} type="number" step="0.01" value={form.charge_rate} onChange={e => set('charge_rate', e.target.value)} /></div>
          </div>
          <div><label className={lbl}>Item *</label><input className={inp} value={form.item} onChange={e => set('item', e.target.value)} required /></div>
          <div><label className={lbl}>Sub-Item</label><input className={inp} value={form.sub_item} onChange={e => set('sub_item', e.target.value)} /></div>
          ...
          <div className="flex items-center gap-6">
            <label className="flex items-center gap-2 cursor-pointer"><input type="checkbox" checked={form.taxable} onChange={e => set('taxable', e.target.checked)} className="w-4 h-4 accent-orange-500" /><span className="text-sm text-gray-300">Taxable</span></label>
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button type="submit" disabled={saving} className="flex-1 bg-orange-500 hover:bg-orange-600 disabled:opacity-50 text-white rounded-lg py-2.5 text-sm font-medium">
              {saving ? <Loader2 className="w-4 h-4 animate-spin mx-auto" /> : isEdit ? 'Save' : 'Add Item'}
            </button>
          </div>
        </form>
      </div>
